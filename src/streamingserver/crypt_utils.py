            ...
            try:
                ...
                if key_iv:
                    try:
                        ...
                        logger.debug("❌ Error converting IV to bytes: %s", e)
                        segment_key["IV"] = None

            except Exception as e:
                logger.debug("❌ Error fetching AES key: %s", e)
...
def download_encryption_key(session, key_url):
    ...
    try:
        ...
        if len(key_data) == 16:
            return key_data
        logger.debug("⚠ Unexpected key length: %s bytes (expected 16)", len(key_data))
        return key_data  # Return anyway, might still work

    except Exception as e:
        logger.debug("❌ Error downloading encryption key: %s", e)
        return None
...
def decrypt_segment(encrypted_data, segment_sequence, media_sequence_base, current_key):
    ...
    try:
        ...
        try:
            ...
            logger.debug("❌ Unpadding failed for segment %s: %s", segment_sequence, e)
            return None
        ...
        logger.debug("❌ Error decrypting segment: %s", e)
        logger.debug("   Key: %s", current_key.hex() if current_key else None)
        logger.debug("   IV: %s (seq: %s)", current_key["IV"].hex() if current_key["IV"] else 'derived', segment_sequence)
        traceback.print_exc()
