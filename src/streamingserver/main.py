...
class PlutoTVRecorder:
        ...
        try:
            while self.is_running and not self._stop_event.is_set():
                ...
                for current_uri, segment_encryption_info, current_duration in uri_list:
                    ...
                    # Check if PTS is continuous
                    diff = first_pts - previous_pts
                    if (first_pts - previous_pts > 2 * previous_duration) or (previous_pts - first_pts > 2 * previous_duration):
                        print(f"⚠️ PTS discontinuity detected: {diff} (previous: {previous_pts}, current: {first_pts})")
                        # Force discontinuity segment
    ...
    def start(self, channel, rec_file):
        ...
        return self.record_stream(channel_uri, rec_file)
...
class RecorderCommandHandler(socketserver.BaseRequestHandler):
    def handle(self):
        try:
            ...
            if cmd == "start":
                if self.server.recorder.is_running:
                    self.server.recorder.stop()
                # Pass both channel and rec_file as a tuple in args
                threading.Thread(target=self.server.recorder.start, args=(req.get("channel"), req.get("rec_file")), daemon=True).start()
                response = {"status": "ok", "message": "Recording started."}

            elif cmd == "stop":
                if self.server.recorder.is_running:
                    self.server.recorder.stop()
                    response = {"status": "ok", "message": "Recording stopped."}
                else:
                    response = {"status": "ok", "message": "Not recording."}
            elif cmd == "status":
                response = {"status": "ok", "recording": self.server.recorder.is_running, "channel": self.server.recorder.channel_uri}
...
def main():
    ...
    try:
        if args.server:
            ...
            print("🚀 Ready for commands. Use 'start', 'stop', or 'status' via socket.")
            while True:
                time.sleep(1)
        elif args.channel:
