        ...
        try:
            while not self._stop_event.is_set():
                ...
                for current_uri, segment_encryption_info, current_duration in uri_list:
                    ...
                    logger.debug("Segment URI list 2: %s: %s", segment_index, segment_encryption_info.get('METHOD', 'none'))
                    logger.debug("Segment URI list 3: %s: %s", segment_index, current_duration)
                    ...
                    segment_data = download_segment(self.session, current_uri, segment_index, segment_encryption_info, max_retries=10, timeout=10)
                    ...
                    current_resolution, current_duration, current_pts = get_segment_properties(segment_data)
