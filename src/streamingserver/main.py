...
from ts_utils import TS_PACKET_SIZE, shift_segment, read_pts_from_segment, is_valid_ts_segment, set_discontinuity_segment, segment_has_keyframe, find_gop_start_in_segment
...
class HLS_Recorder:
    ...
    def append_to_ts(self, rec_file, segment_data, current_uri, segment_index):
        ...
        try:
            ...
            print("=" * 70)
            print(f"✓ Appended segment {segment_index}, {uri_name} to {rec_file} {file_size:.2f} MB")
            print("=" * 70)
            return True
    ...
    def record_stream(self, channel_uri, rec_file):
        """Main recording"""
        print("🎬 HLS Recorder with timestamp recalculation")
        print(f"channel_uri: {channel_uri}, rec_file: {rec_file}")
            ...
            while not self._stop_event.is_set():
                ...
                if not playlist:
                    ...
                    if failed_playlist_count >= max_failed_playlists:
                        print("❌ Too many failed playlist fetches. Aborting...")
                ...
                if not uri_list:
                    ...
                    if empty_playlist_count >= max_empty_playlists:
                        print("❌ Playlist has been empty for too long. Aborting...")
                ...
                for current_uri, segment_encryption_info, current_duration in uri_list:
                    ...
                    # wait for first keyframe to be available in stream
                    if not keyframe and not "_plutotv_error_" in current_uri:
                        packet_index, _vid_pid, pat_pkt, pmt_pkt = find_gop_start_in_segment(segment_data)
                        if packet_index == -1:
                            print(f"⚠ Segment {segment_index} does not start with keyframe, skipping")
                            continue
                        self.append_to_ts(rec_file, pat_pkt, "PAT", segment_index)
                        self.append_to_ts(rec_file, pmt_pkt, "PMT", segment_index)
                    keyframe = True

                    # strip segment data to start with keyframe
                    if segment_index == 0 and packet_index > 0:
                        print(f"⚠ Segment {segment_index} starts with keyframe at packet index {packet_index}, stripping to start with keyframe")
                        segment_data = segment_data[packet_index:]

                    current_pts, _ = read_pts_from_segment(segment_data)
                    print(f"Segment {segment_index} PTS: {current_pts}")
                    if current_pts is None:
                        print(f"❌ No PTS found in segment {segment_index}")
                        raise ValueError(f"No PTS found in segment {segment_index}")

                    # recalculate PTS in case first segment was stripped
                    if segment_index == 1:
                        print(f"Segment {segment_index} recalculating hls duration: {current_duration}")
                        current_duration = current_pts - previous_pts
                        print(f"Segment {segment_index} recalculated duration: {current_duration}")

                    continuous_pts += previous_duration
                    offset = continuous_pts - current_pts
                    segment_data = shift_segment(segment_data, offset)
                    print(f"### Previous PTS: {previous_pts}, Previous duration: {previous_duration}, Current PTS: {current_pts}, Continuous PTS: {continuous_pts}, Offset: {offset}")

                    # Check if segment URI has changed
                    if previous_uri and different_uris(previous_uri, current_uri):
                        print(f"Changing URI from: {previous_uri}")
                        print(f"Changing URI to:   {current_uri}")
                        segment_data = set_discontinuity_segment(segment_data, force=False)
    ...
    def start(self, channel, rec_file):
        ...
        print(f"📺 Using channel URI: {channel_uri}")
        if self.is_running:
            self.stop()
