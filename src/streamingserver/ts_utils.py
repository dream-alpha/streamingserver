...
def make_ts_header(pid: int, payload_unit_start: bool = False, continuity_counter: int = 0) -> bytes:
    sync_byte = 0x47
    tei_pusi_tp = 0x00
    ...
    return bytes([
        ...
        tei_pusi_tp | pid_hi,
        pid_lo,
        0x10 | (continuity_counter & 0x0F)  # payload only
    ])


def crc32_mpeg(data: bytes) -> int:
    # ISO/IEC 13818-1 CRC32
    crc = 0xFFFFFFFF
    for b in data:
        crc ^= b << 24
        for _ in range(8):
...
def make_pat(pmt_pid: int = 0x100) -> bytes:
    # Table ID 0x00 (PAT)
    section = bytes([
        0x00,        # table_id
    ...
    crc = struct.pack(">I", crc32_mpeg(section))
    payload = section + crc
    pointer_field = b"\x00"
    stuffing = bytes([0xFF] * (TS_PACKET_SIZE - 4 - len(pointer_field) - len(payload)))
    return make_ts_header(0x0000, payload_unit_start=True) + pointer_field + payload + stuffing


def make_pmt(pcr_pid: int = 0x100, video_pid: int = 0x100, audio_pid: int = 0x101) -> bytes:
    # PMT table_id 0x02
    section = bytes([
        0x02,        # table_id
        ...
        0x00,        # section_number
        0x00,        # last_section_number
        0xE0 | ((pcr_pid >> 8) & 0x1F), pcr_pid & 0xFF,  # PCR PID
    ...
    pointer_field = b"\x00"
    stuffing = bytes([0xFF] * (TS_PACKET_SIZE - 4 - len(pointer_field) - len(payload)))
    return make_ts_header(0x0100, payload_unit_start=True) + pointer_field + payload + stuffing


def make_eos_ts_packet(pid: int, stream_id: int) -> bytes:
    ts_header = make_ts_header(pid, payload_unit_start=True)
    pes_header = bytes([
        0x00, 0x00, 0x01,
        stream_id & 0xFF,
        0x00, 0x00,
    ...
    payload = pes_header + bytes([0xFF] * (184 - len(pes_header)))
